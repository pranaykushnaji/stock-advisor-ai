...
const STORAGE_KEYS = { KEY: 'sa_api_key', BOUQUET: 'sa_bouquet', HISTORY: 'sa_history', PROVIDER: 'sa_provider' };
...
function getProvider() { return localStorage.getItem(STORAGE_KEYS.PROVIDER) || 'groq'; }
function saveProvider(p) { localStorage.setItem(STORAGE_KEYS.PROVIDER, p); }
...
function updateModalHint() {
  ...
  if (p === 'groq') {
    hint.innerHTML = '<ol class="setup-steps"><li>Go to <a href="https://console.groq.com/keys" target="_blank">console.groq.com/keys</a></li><li>Sign up free (Google/GitHub login)</li><li>Click <strong>"Create API Key"</strong></li><li>Copy & paste below (starts with <code>gsk_</code>)</li></ol>';
  } else {
    hint.innerHTML = '<ol class="setup-steps"><li>Go to <a href="https://aistudio.google.com/apikey" target="_blank">aistudio.google.com/apikey</a></li><li>Sign in with Google</li><li>Click <strong>"Create API Key"</strong></li><li>Copy & paste below</li></ol>';
  }
}

if (providerSelect) providerSelect.addEventListener('change', updateModalHint);
...
document.getElementById('save-key-btn').addEventListener('click', () => {
  ...
  if (p === 'groq' && !k.startsWith('gsk_')) { keyError.textContent = 'Groq keys start with "gsk_" — check your key.'; return; }
  saveKey(k);
  saveProvider(p);
  closeModal();
...
// ─── Analysis prompt ───
const ANALYSIS_PROMPT = `You are an expert stock market analyst specializing in Indian and global equities. Analyze this stock for investment.
...
Return ONLY a valid JSON object — no markdown, no explanation, no text before or after. Exactly this structure:
{
  ...
  "priceContext": "Current price range, P/E, valuation note"
}
...
async function analyzeStock() {
  ...
  try {
    ...
    if (provider === 'groq') {
      rawText = await callGroq(key, input);
    } else {
      rawText = await callGemini(key, input);
    }
    ...
    if (!parsed.ticker || !parsed.verdict) throw new Error('Incomplete analysis. Try again.');
    ...
    area.innerHTML = `<div class="error-card">
      ...
      ${err.message.includes('key') || err.message.includes('auth') || err.message.includes('401') ?
        `<br><button class="btn btn-ghost" onclick="openModal()" style="margin-top:8px;">Update API Key</button>` : ''}
    </div>`;
  } finally {
    btn.disabled = false;
...
// ─── Groq API call ───
async function callGroq(key, stockName) {
  const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        { role: 'user', content: `Analyze this stock for investment: "${stockName}"` }
      ],
      temperature: 0.4,
      max_tokens: 1200
    })
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) throw new Error('Invalid Groq API key. Click API Settings to update.');
...
// ─── Gemini API call ───
async function callGemini(key, stockName) {
  const models = ['gemini-2.0-flash-lite', 'gemini-2.0-flash'];
  const fullPrompt = ANALYSIS_PROMPT + `\n\nAnalyze this stock: "${stockName}"`;
  ...
  throw new Error('All Gemini models rate-limited. Wait 60 seconds or switch to Groq in API Settings.');
}
...
function approvePick() {
  ...
  note.innerHTML = `<span style="font-size:20px">✅</span><span><strong>${esc(currentAnalysis.ticker)}</strong> added. Switch to <strong>My Bouquet</strong> to track it.</span>`;
  area.appendChild(note);
}

function skipPick() {
  currentAnalysis = null;
  document.getElementById('result-area').innerHTML = '<p style="color:var(--text3);padding:8px 0;font-size:14px;">Skipped. Enter another stock.</p>';
}
    ...
    <div class="bouquet-list">
      ${bouquet.map((item, i) => {
        ...
        return `<div class="bouquet-item">
          ...
          <button class="bi-remove" onclick="removePick(${i})" title="Remove">✕</button>
        </div>`;
      }).join('')}
      <div class="sim-note">Simulated returns only — not real portfolio data.</div>
    </div>`;
}
  ...
  setTimeout(() => {
    document.getElementById('result-area').innerHTML = `<div class="result-card" style="border-left:3px solid var(--accent);display:flex;align-items:flex-start;gap:16px;">
      ...
      <div>
        ...
        <div style="font-size:13px;color:var(--text2);margin-bottom:14px;"><strong>Recommended: Groq</strong> — free, fast, 14,400 requests/day. No credit card.<br>Get your key at <a href="https://console.groq.com/keys" target="_blank" style="color:var(--accent);">console.groq.com/keys</a></div>
